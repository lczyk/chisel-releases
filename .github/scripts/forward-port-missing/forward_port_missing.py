...
def load_data(
    ...
    with NamedTemporaryFile(delete=False, suffix=".db") as tmp:
        ...
        ubuntu_releases: set[UbuntuRelease] = {
            UbuntuRelease(version=row[0], codename=row[1], supported=bool(row[2]), devel=bool(row[3]))
        ...
        ubuntu_releases = set(release for release in ubuntu_releases if release.supported)
